...
export const FORTA_PROXY_AGENT_REGISTRY_CORE = "0x61447385B019187daa48e91c55c02AF1F1f3F863";
...
export const providerHandleTransaction = (handlerInputs: HandlerInputs): HandleTransaction => {
  return async (txEvent: TransactionEvent) => {
    const { DEPLOYER_ADDRESS, FORTA_PROXY_AGENT_REGISTRY_CORE, CREATE_AGENT_METHOD } = handlerInputs;
    const findings: Finding[] = [];
    if (txEvent.from !== DEPLOYER_ADDRESS.toLowerCase()) return findings;
    const createAgentMethodCalls = txEvent.filterFunction(CREATE_AGENT_METHOD, FORTA_PROXY_AGENT_REGISTRY_CORE);
